...
class JustOneGame {
    ...
    async initialize() {
        ...
        if (!Array.isArray(allCards) || allCards.length < 13) {
            throw new Error("Le deck est trop petit ou mal formaté.");
        }

        // Mélanger et sélectionner 13 cartes
        this.deck = this.shuffleArray(allCards).slice(0, 13);
    ...
    async playRound() {
        ...
        const wordIndex = await this.askNumber(`${activePlayer}, choisissez un numéro entre 1 et 5 : `);
        ...
        return new Promise(resolve => {
            const askAgain = () => {
                rl.question(question, (answer) => {
                    if (!/^[a-zA-ZÀ-ÖØ-öø-ÿ]+$/.test(answer)) {
                        console.log("Erreur : Votre réponse ne doit contenir que des lettres sans espaces ni caractères spéciaux. Essayez encore.");
                        askAgain();
                    } else {
            ...
            const askAgain = () => {
                rl.question(question, (answer) => {
                    ...
                    if (!Number.isInteger(number) || number < 1 || number > 5) {
                        console.log("Erreur : Veuillez entrer un nombre entre 1 et 5.");
                        askAgain();
                    } else {
                        resolve(number);
                    }
